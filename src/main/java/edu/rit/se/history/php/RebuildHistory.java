...
import org.chaoticbits.devactivity.parse.FileListingParser;
...
//		log.info("Done.");
	...
	// export.add(props.getProperty("history.cves.googledoc"), new File(datadir,
	// props.getProperty("history.cves.local")));
	// export.add(props.getProperty("history.cve2files.googledoc"), new File(datadir,
	// props.getProperty("history.cve2files.local")));
	// export.add(props.getProperty("history.groundedtheory.googledoc"), new File(datadir,
	...
	// }
	//
	// private void loadFileListing(DBUtil dbUtil, Properties props) throws FileNotFoundException,
	// SQLException {
	// log.info("Parsing release files for Tomcat 5.5.0...");
	// new FileListingParser().parse(dbUtil, new File(datadir, props.getProperty("history.filelisting.v5")),
	// "5.5.0");
	// log.info("Parsing release files for Tomcat 6.0.0...");
	// new FileListingParser().parse(dbUtil, new File(datadir, props.getProperty("history.filelisting.v6")),
	// "6.0.0");
	// log.info("Parsing release files for Tomcat 7.0.0...");
	// new FileListingParser().parse(dbUtil, new File(datadir, props.getProperty("history.filelisting.v7")),
	// "7.0.0");
	...
	// }
	//
	// private void buildAnalysis(DBUtil dbUtil, Properties props) throws FileNotFoundException,
